...
export default function AllPostDropdown({ onSelect }) {
  const { dark } = useTheme(); // Access the current theme
  const [selectedValue, setSelectedValue] = useState("html");
  const [isOpen, setIsOpen] = useState(false);

  const options = [
    { value: "blog_channel", label: "Blog Channel" },
    { value: "interview_channel", label: "Interview Channel" },
  ...
  return (
    <div
      className={`w-[70%] space-y-2 relative ${
        dark ? "text-black bg-white rounded-md" : "text-white"
      } shadow-lg`}
    >
      <div
      ...
      >
        <div
          ...
          className={`outline-none flex items-center justify-between px-3 py-2 duration-300 ease-in-out ${
            dark ? "outline-none" : "bg-black outline-none"
          }`}
        >
