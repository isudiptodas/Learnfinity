  ...
  const createPost = async (e) => {
      ...
      try {
        ...
        if (postImage !== null) {
          ...
          formData.append('upload_preset', 'Learnfinity-blog-post');
          formData.append('folder', 'Blog post image');
        ...
        const res = await axios.post(`https://learnfinity-mzah.onrender.com/api/create/blog/post`, {
        ...
        setTimeout(() => {
          navigate('/community/blog-channel');
        }, 3000);
      }
      catch (err) {
        ...
        <div className={`z-20 ${dark ? "bg-gray-200 duration-200 ease-in-out" : "bg-zinc-900 duration-200 ease-in-out"} py-5 w-full h-[83vh] lg:h-auto lg:py-5 px-10 rounded-xl md:py-5 content overflow-auto flex flex-col justify-start items-center gap-10 relative lg:gap-6`}>
          ...
          <form className={`h-auto w-full md:w-[70%] lg:w-[50%] px-5 py-5 flex flex-col justify-start items-center lg:items-start gap-5`}>
            ...
            <div className='w-full pr-3 py-4 flex flex-col justify-center items-start gap-3'>
              ...
              <input type="text" className={`w-full rounded-md px-5 py-2 ${dark ? "bg-white text-black" : "bg-black text-white"} duration-300 ease-in-out`} placeholder='Enter post title' onChange={(e) => setTitle(e.target.value)}/>
            </div>

            <div className='w-full pr-3 py-4 flex flex-col justify-center items-start gap-3'>
              <label htmlFor="desc" className={`${dark ? "text-black" : "text-white"}`}>Description : </label>
              <textarea className={`w-full rounded-md px-5 py-4 min-h-40 lg:min-h-60 ${dark ? "bg-white text-black" : "bg-black text-white"} duration-300 ease-in-out`} placeholder='Enter post description' onChange={(e) => setDesc(e.target.value)}/>
            </div>

            <div className='w-full pr-3 py-4 flex flex-col justify-center items-start gap-3'>
              <label htmlFor="tags" className={`${dark ? "text-black" : "text-white"}`}>Tags : </label>
              <input type="text" className={`w-full rounded-md px-5 py-4 ${dark ? "bg-white text-black" : "bg-black text-white"} duration-300 ease-in-out`} placeholder='Add tag name' onChange={handleTagName} value={tagName} />
              ...
              <button className={`w-full mt-12 py-3 px-4 bg-gradient-to-br from-cyan-400 to-cyan-700 text-white font-bold cursor-pointer rounded-md`} onClick={createPost}>{isAddingPost ? (<div>Adding post ... <Loading /></div>) : (<div>Add post</div>)}</button>
            </div>
