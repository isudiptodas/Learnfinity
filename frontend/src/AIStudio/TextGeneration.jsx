...
function TextGeneration() {
  ...
  // const edenURL = "https://api.edenai.run/v2/text/generation";
  ...
  const handleGenerateText = async () => {
    if (!text) {
      toast.error("Please provide a prompt for text generation!");
      return;
    }
    ...
    try {
      ...
      // }
      // const generated = res.data["cohere/command-nightly"]?.generated_text;
      ...
      if (res.data) {
        setGeneratedText(res.data.candidates[0]?.content.parts[0]?.text);
      }
      //console.log(res.data);
      ...
      toast.error("Error generating text. Please try again later.");
    } finally {
  ...
  const saveHistory = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/save/text', {
        text, generatedText
      }, {
        headers: {
  ...
  const fetchHistory = async (e) => {
    ...
    try {
      ...
      const res = await axios.get('http://localhost:5000/history/text', {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const deleteHistory = async (text) => {
    try {
      const res = await axios.delete('http://localhost:5000/delete/text', {
        params: {
          id: text._id,
        },
      });
        ...
        <div className={`z-20 ${dark ? "bg-gray-200 duration-200 ease-in-out" : "bg-zinc-900 duration-200 ease-in-out"} py-5 w-full h-[83vh] lg:h-auto px-10 rounded-xl md:py-5 lg:py-10 content overflow-auto flex flex-col justify-start items-start md:items-center gap-10 lg:gap-16 relative`}>
          ...
          <div className={`w-full md:w-2/3 h-auto rounded-lg bg-transparent ${dark ? "bg-white shadow-lg" : "bg-black"} `}>
            <div className={`w-full text-s py-3 lg:text-sm overflow-x-auto flex justify-start md:justify-evenly items-center px-5 gap-7 flex-shrink-0 content ${dark ? "text-black" : "text-white border-b-2 border-gray-500"}`}>
              ...
              <Link to="/ai-studio/text-generation" className='flex-shrink-0 text-center cursor-pointer bg-cyan-500 px-3 py-2 rounded-md md:w-[20%] hover:bg-cyan-700 duration-200 ease-in-out text-white'>Text Generation</Link>
          ...
          <div className={`w-full flex justify-between items-start gap-5 border-t-[1px] border-gray-500 py-5`}>
            ...
            <div className={`hidden lg:block w-[25%] px-5 py-5 h-125 ${dark ? "bg-gray-300" : "bg-black"} overflow-y-auto content flex flex-col justify-start items-center rounded-lg`}>
              ...
              {history.length > 0 ? (history.map((data) => (
                <div key={data._id} className={`w-full text-s md:text-sm rounded-lg py-5 mt-5 mb-5 ${dark ? "bg-white text-black" : "bg-zinc-700 text-white"} flex justify-between items-center duration-300 ease-in-out px-5 h-auto cursor-pointer`} onClick={() => loadGeneratedText(data)}>
                  {truncateText(data.query, 20)} <FaTrashAlt className='text-red-500 cursor-pointer text-sm' onClick={() => deleteHistory(data)} />
                </div>
            ...
            <div className='h-auto py-2 w-full flex flex-col justify-center items-center'>
              ...
              <textarea className={`${dark ? "bg-white text-black" : "bg-black text-white"} w-full py-3 h-44 px-5 md:py-4 md:px-7 text-s sm:text-sm rounded-md`} placeholder='Enter your thought' value={text} onChange={handleChange} />

              <div className='w-full h-auto flex justify-center items-center gap-5 lg:gap-10'>
                <button className='w-full flex justify-center items-center gap-4 bg-gradient-to-r bg-cyan-600 py-2 md:py-4 rounded-md mt-5 mb-10 text-white cursor-pointer duration-300 ease-in-out font-Titillium font-semibold' onClick={handleGenerateText}>{isGenerating ? (<>Generating, please wait... <Loading /></>) : (<>Generate Text </>)} </button>
                ...
                <button className={`w-1/2 flex justify-center items-center gap-4 py-2 md:py-4 rounded-md mt-5 mb-10 text-white bg-emerald-600 cursor-pointer duration-300 ease-in-out font-Titillium font-semibold hover:opacity-85 ${generatedText === '' ? "hidden" : "block"}`} onClick={saveHistory}>Save ✓</button>
              </div>
              <div className={`${isHistoryVisible ? "hidden" : "block"} flex flex-col gap-5 ${dark ? "bg-gradient-to-b from-gray-400 to-transparent text-black" : "bg-gradient-to-b from-zinc-700 to-transparent text-white"} h-auto py-3 px-4 text-black w-full rounded-t-xl text-s md:text-sm text-start ${generatedText === '' ? "hidden" : "block"} relative overflow-hidden `}>
                {generatedText}
              ...
              <div className={`${isHistoryVisible ? "block" : "hidden"} w-full h-auto py-5 px-5 overflow-y-auto ${dark ? "bg-gradient-to-b from-white to-transparent" : "bg-gradient-to-b from-black to-transparent"} rounded-lg flex flex-col justify-start items-center gap-5`}>
                {history.length > 0 ? (history.map((data) => (
                  <div key={data._id} className={`w-full text-s md:text-sm rounded-lg py-5 ${dark ? "bg-gray-300 text-black" : "bg-zinc-700 text-white"} flex justify-between items-center duration-300 ease-in-out px-5 h-auto cursor-pointer`} onClick={() => loadGeneratedText(data)}>
