...
function LoginPage() {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            setRegistering(true);
            const res = await axios.post("https://learnfinity-mzah.onrender.com/auth/register", { name, displayName, email, password });
            ...
            <div className='min-h-[100vh] w-full bg-white flex flex-col lg:flex-row gap-10 items-center justify-center relative overflow-y-auto py-24 lg:py-5 '>
                ...
                <div className='border-2 border-cyan-600 w-[80%] sm:w-[60%] lg:w-1/3 py-10 z-50 h-auto rounded-3xl mt-[500px] sm:mt-[500px] md:mt-[550px] lg:mt-0 lg:absolute lg:right-24'>
                    ...
                    <div className=' w-full h-auto py-2 z-40 flex flex-col justify-center items-center'>
                        <h1 className='text-black text-3xl md:text-4xl font-Titillium font-bold'>Unlock growth</h1>
                        <h1 className='text-black text-3xl md:text-4xl font-Titillium font-bold'><span className='font-Ranga mr-5 bg-gradient-to-r from-cyan-400 to-teal-700 bg-clip-text text-transparent'>Register</span>here</h1>
                    </div>

                    <form className=' h-full w-full px-10 flex flex-col justify-center items-start gap-5 text-black' onSubmit={handleRegister}>
                        <div className='flex flex-col w-full'>
                            <label htmlFor="name">Full name *</label>
                            <input type="text" placeholder='Enter full name' className='w-full rounded-md bg-gray-200 text-black px-5 py-2' onChange={(e) => setName(e.target.value)} />
                        </div>

                        <div className='flex flex-col w-full'>
                            <label htmlFor="displayname">Display name *</label>
                            <input type="text" placeholder='Enter display name' className='w-full rounded-md bg-gray-200 text-black px-5 py-2' onChange={(e) => setDisplayName(e.target.value)} />
                        </div>
                        ...
                        <div className='flex flex-col w-full relative'>
                            ...
                            <div className='w-full flex'>
                                ...
                                <span className='opacity-40 absolute top-1/2 right-5 transform cursor-pointer' onClick={toggleVisibility}>{isVisible ? <FaEye/> : <FaEyeSlash/>}</span>
                            </div>
                        </div>
                        ...
                        <div className='flex flex-col w-full relative'>
                            ...
                            <div className='w-full flex'>
                                <input type={isVisible ? "text" : "password"} placeholder='Confirm password' className='w-full rounded-md bg-gray-200 text-black px-5 py-2' onChange={(e) => setConfirmPassword(e.target.value)} />
                                <span className='opacity-40 absolute top-1/2 right-5 transform cursor-pointer' onClick={toggleVisibility}>{isVisible ? <FaEye/> : <FaEyeSlash/>}</span>
                            </div>
                        </div>

                        <button type='submit' className='w-full py-2 bg-black text-white hover:bg-zinc-700 duration-200 ease-in-out text-sm lg:text-lg'>{registering ? "Registering ... please wait" : "Register"}</button>
                    </form>
                </div>
